...
export default function AutostakingDepositPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const newTaskId = `autostaking-deposit-${Date.now()}`
    setTaskId(newTaskId)

    try {
      const response = await fetch('/api/autostaking-deposit', {
        method: 'POST',
        headers: {
      ...
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            Autostaking Deposit Automation
          </h1>
          <p className="text-gray-600">
            Deposit via advisor for autostaking
          </p>
        </div>
