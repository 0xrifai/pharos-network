...
export default function CfdTradingPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const newTaskId = `cfd-trading-${Date.now()}`
    setTaskId(newTaskId)

    try {
      const response = await fetch('/api/cfd-trading', {
        method: 'POST',
        headers: {
      ...
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            CFD Trading Automation
          </h1>
          <p className="text-gray-600">
            Automate CFD trading operations
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              >
                {isRunning ? 'Running...' : 'Start CFD Trading'}
              </button>
            </form>
