...
export default function ZenithWphrsLiquidityPage() {
  ...
  const [slippage, setSlippage] = useState(0.15)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const newTaskId = `zenith-wphrs-liquidity-${Date.now()}`
    setTaskId(newTaskId)

    try {
      const response = await fetch('/api/zenith-wphrs-liquidity', {
        method: 'POST',
        headers: {
      ...
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            Zenith WPHRS Liquidity
          </h1>
          <p className="text-gray-600">
            Add WPHRS liquidity to Zenith Finance
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <LogsDisplay 
            taskId={taskId} 
            title="Zenith WPHRS Liquidity Logs"
